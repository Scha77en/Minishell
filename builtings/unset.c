...
/*   Updated: 2023/09/24 08:41:46 by aouhbi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_unset(char **a, t_cmd *tavern)
{
	t_env	*env;
	t_env	*tmp;
	...
	{
		tavern->env = (env->next) ? env->next : tavern->env;
		free_node(tavern, env);
		return (SUCCESS);
	}
	while (env && env->next)
	{
