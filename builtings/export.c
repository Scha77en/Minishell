...
/*   Updated: 2023/09/29 11:42:44 by aouhbi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_join_value(t_env **env, char **split)
{
	...
	while (current)
	{
		if (ft_strncmp_m(current->var, split[0], ft_strlen(join)) == 0)
		{
...
void	ft_add_env(t_env **env, char **split)
{
	...
	if (plus_sign(split[0], 1))
		new->var = ft_strndup(split[0], plus_sign(split[0], 1) - 1);
	else
